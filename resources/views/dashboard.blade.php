            ...
            <aside class="glass w-64 min-h-screen p-4 text-white rounded-lg">
                ...
                <nav>
                    <ul class="space-y-2">
                        ...
                        <li>
                            <a href="#" class="flex items-center p-3 rounded-lg hover:bg-blue-600 transition-all" onclick="showSection('socials')">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div id="posts" class="content-section active">
                    <div class="glass rounded-xl p-6 text-white">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <a href="{{ route('admin.posts.index') }}" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg flex items-center">
                               
                                Tous les posts
                            </a>
                            <a href="{{ route('admin.posts.create') }}" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg flex items-center">
            ...
            <table class="w-full overflow-x-auto">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-700 hover:bg-blue-900/50">
                        ...
                        <td class="px-4 py-3">
                            @if($user->role === 0)
                                <span class="px-2 py-1 bg-red-500 rounded-full text-xs">Admin</span>
                            @else
                                <span class="px-2 py-1 bg-blue-500 rounded-full text-xs">Utilisateur</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex space-x-2">
                                @if(auth()->user()->id !== $user->id)
                                    <form id="role-form-{{ $user->id }}" action="{{ route('admin.toggleUserRole', ['user' => $user]) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="button" onclick="confirmRoleChange({{ $user->id }}, '{{ $user->name }}', {{ $user->role }})" class="{{ $user->role === 0 ? 'text-red-400 hover:text-red-300' : 'text-blue-400 hover:text-blue-300' }}">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.deleteUser', ['user' => $user]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $user->id }}, '{{ $user->name }}')" class="text-red-400 hover:text-red-300">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                            </svg>
                                        </button>
...
<div id="socials" class="content-section">
    <div class="glass rounded-xl p-6 text-white">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.socials.create') }}" class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-lg">
            ...
            <table class="w-full overflow-x-auto">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-700 hover:bg-blue-900/50">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ $social->url }}" target="_blank" class="text-blue-400 hover:text-blue-300">
                                {{ $social->url }}
                            </a>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.socials.edit', $social) }}" class="text-blue-400 hover:text-blue-300">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                    </svg>
                                </a>
